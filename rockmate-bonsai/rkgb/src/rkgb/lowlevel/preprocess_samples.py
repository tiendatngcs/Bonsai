...
class ExampleInputs():
    def __init__(self,original_mod,original_mod_args,original_mod_kwargs=None):
        ...
        if isinstance(original_mod_args,dict):
            print(f" original_mod_args is a dict with keys {list(original_mod_args.keys())}")
            ...
            if "input_ids" in dict_example_inputs:
                print(" Detected 'input_ids' in original_mod_args, assuming LLM input format.")
                # inputs = [dict_example_inputs["input_ids"], dict_example_inputs["attention_mask"]]
                inputs = {
                    "input_ids": dict_example_inputs["input_ids"],
                    # "attention_mask": dict_example_inputs["attention_mask"]
                }
                # nb_given = len(inputs)
                ...
                dict_example_inputs = inputs
                print(f"dict_example_inputs keys: {list(dict_example_inputs.keys())}")
                self.args = (inputs,)
                self.dict.update(dict_example_inputs)
                self.is_llm_input = True
        else:
