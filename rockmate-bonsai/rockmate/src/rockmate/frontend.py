...
def generate_config(config_type):
    ...
    elif config_type == "segment_ilp":
        _add_top_solver(result, "hilp")
        result.solver.top.hilp.time_limit = 3600
        result.solver.top.hilp.nb_total_nodes = 10000
        # set environment variable CUSTOM_ILP_SOLVER=1
        os.environ["CUSTOM_ILP_SOLVER"] = "1"
    elif config_type == "greedy-search":
        _add_top_solver(result, "hilp")
        result.solver.top.hilp.time_limit = 3600
        result.solver.top.hilp.nb_total_nodes = 10000
        # set environment variable GREEDY_SEARCH=1
        os.environ["GREEDY_SEARCH"] = "1"
    elif config_type == "recompute-op-list":
        _add_top_solver(result, "hilp")
        result.solver.top.hilp.time_limit = 3600
        result.solver.top.hilp.nb_total_nodes = 10000
        # set environment variable RECOMPUTE_OP_LIST=1
        os.environ["RECOMPUTE_OP_LIST"] = "1"
    elif config_type == "no-recompute":
        _add_top_solver(result, "hilp")
        result.solver.top.hilp.time_limit = 3600
        result.solver.top.hilp.nb_total_nodes = 10000
        # set environment variable CUSTOM_ILP_SOLVER=1
        os.environ["NO_RECOMPUTE"] = "1"
    elif config_type == "offload_all":
        _add_top_solver(result, "hilp")
        result.solver.top.hilp.time_limit = 3600
        result.solver.top.hilp.nb_total_nodes = 10000
        # set environment variable OFFLOAD_ALL=1
        os.environ["OFFLOAD_ALL"] = "1"
...
def from_config(model, model_inputs, budget=None, config_type="hiremate", config=None, cache_sched=False, **kwargs):
    if config is None:
    ...
    return Rockmate(model, model_inputs, budget, top_solvers=top_solvers, bottom_solvers=bottom_solvers,
                    partitioners=partitioners, cache_sched=cache_sched, **kwargs)
...
class PureCheckmate(Rockmate):
    ...
    def solver(cls):
        ...
        hilp_solver = solvers.HILP(ilp_solver="GUROBI_CMD", offload=False)
        return hilp_solver
...
class Offmate(Rockmate):
    ...
    def __init__(self, model, model_inputs, nlayers=32, budget=None, algorithms=valid_algorithms, partitioner_params={}, **kwargs):
        ...
        solver = solvers.HILP(ilp_solver="GUROBI_CMD")
        solver.config.offload = True
        solver.config.activation_offload = True
