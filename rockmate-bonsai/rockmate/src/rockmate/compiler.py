...
accumulated_offload_bytes = 0

def write_to_csv(type_op, par_name, time_taken, mem): 
    csv_file = f"timing_log.csv"
    with open(csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)

        if file.tell() == 0: 
            writer.writerow(["type", "parameter", "time_taken_ms", "memory(MB)"])
        epoch = 1
        writer.writerow([type_op,par_name ,time_taken, mem])
...
class RK_Storage:
    ...
    def add_val(self, target, x):
        # print(f"Value of target {target} updated from shape {self.ld[target].shape if target in self.ld else 'N/A'} to shape {x.shape if isinstance(x, torch.Tensor) else 'N/A'}")
        self.ld[target] = x

    def get_val(self, target):
        if target in self.manager.target_allocations:
            # print(f"Target {target} is in allocation {self.manager.target_allocations[target]}, retrieving from allocation")
            return self.manager.get_val(target)
        if target in self.ld:
            # for tensors
            # print(f"Retrieving {target} from local dictionary with shape {self.ld[target].shape} and dtype {self.ld[target].dtype}")
            return self.ld[target]
        elif target in self.gd:
            # print(f"Retrieving {target} from global dictionary with shape {self.gd[target].shape if isinstance(self.gd[target], torch.Tensor) else 'N/A'} and dtype {self.gd[target].dtype if isinstance(self.gd[target], torch.Tensor) else 'N/A'}")
...
class Fct_del(RK_Fct):
    ...
    def __call__(self):
        # print(f"DEL: {self.target_name} - {self.del_mode}")
...
class Fct_run_fwd(RK_Fct):
    ...
    def fwd_with_grad(self):
        # print(f"Executing Fwd_with_grad for {self.target_name}")
    ...
    def __call__(self):
        # print(f"FWD: {self.target_name}")
        # with torch.cuda.stream(self.storage.gd[self.stream]):
        # self.fwd_fct[self.fwd_mode]()

        if self.is_backward:
            mem_before = 0
            for tensor_dict in [self.storage.gd, self.storage.ld]:
                for v in tensor_dict.values():
                    if isinstance(v, torch.Tensor):
                        mem_before += v.numel() * v.element_size()
            ...
            for tensor_dict in [self.storage.gd, self.storage.ld]:
                for v in tensor_dict.values():
                    if isinstance(v, torch.Tensor):
                        mem_after += v.numel() * v.element_size()

            mem_delta = mem_after - mem_before
            mem_delta_mb = mem_delta / (1024 ** 2)
            # write_to_csv("recomputation", self.target_name, time_taken, mem_delta_mb)
        else: 
            self.fwd_fct[self.fwd_mode]()
...
class Fct_mem_alloc(RK_Fct):
    ...
    def alloc_grad(self):
        ...
        # )
        param = self.storage.get_val(self.target_name)

        # Remove pin_memory if param is not on CPU
        kwargs = self.kwargs.copy()
        if param.device.type != "cpu":
            kwargs.pop("pin_memory", None)  # Safely remove it if present

        param.grad = torch.empty(shape, dtype=dtype, device=param.device, **kwargs)

    def alloc_data(self):
        assert self.device.type !='cpu', "CPU data should not reach here"
        print(f"Allocating data for {self.target_name} on device {self.device} with shape {self.storage.get_shape(self.shape_name)} and dtype {self.storage.dtypes.get(self.shape_name, self.dtype)}")
        shape = self.storage.get_shape(self.shape_name)
        dtype = (
            self.storage.dtypes[self.shape_name] if self.shape_name in self.storage.dtypes else self.dtype
        )
    ...
    def alloc_tensor(self):
        # print(f"Allocating tensor for {self.target_name} on device {self.device} with shape {self.storage.get_shape(self.shape_name)} and dtype {self.storage.dtypes.get(self.shape_name, self.dtype)}")
        shape = self.storage.get_shape(self.shape_name)
        dtype = (
            self.storage.dtypes[self.shape_name] if self.shape_name in self.storage.dtypes else self.dtype
        )
        # use pin memory if device is CPU
...
class Fct_offload(RK_Fct):
    def __init__(
        ...
        **kwargs,
        ...
        self.stream = stream
        # print(f"Initialized offload function for {self.target_name} with mode {self.offload_mode} on stream {self.stream}")
    ...
    def offload_tensor(self):
        ...
        #     non_blocking=True,
        # )
        gpu_tensor = self.storage.get_val(self.target_name)
        # If gpu tensor is empty then skip offloading
        if gpu_tensor.numel() == 0:
            # print(f"Skipping offloading {self.target_name} as GPU tensor is empty")
            return
        # create empty CPU tensor with same size as GPU tensor
        cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}")
        # if cpu_tensor size is [0], create a new tensor with the shape of gpu_tensor
        if cpu_tensor.numel() == 0:
            self.storage.add_val(f"cpu_{self.target_name}", torch.empty(gpu_tensor.shape, dtype=gpu_tensor.dtype, device="cpu", pin_memory=True))
            cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}")
        
        # self.storage.add_val(f"cpu_{self.target_name}", gpu_tensor.to("cpu", non_blocking=True))
        # self.storage.add_val(self.target_name, torch.empty(0, dtype=gpu_tensor.dtype, device=gpu_tensor.device))
        
        # # synchronize here and verify the data is on CPU
        # torch.cuda.synchronize()
        # cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}")
        # print(f"Offloaded {self.target_name} to CPU, CPU_tensor_shape {cpu_tensor.shape}, GPU_tensor_shape {gpu_tensor.shape}")
        # time.sleep(1)  # simulate some delay for offloading

        # cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}").data
        # gpu_tensor = self.storage.get_val(self.target_name).data
        # print(f"Offloading {self.target_name} to CPU, CPU_tensor_shape {cpu_tensor.shape}, GPU_tensor_shape {gpu_tensor.shape}")
        tensor_size_bytes = gpu_tensor.numel() * gpu_tensor.element_size()
        # tensor_size_mb = tensor_size_bytes / (1024 ** 2) 
        # start_time = time.time()
        # cpu_tensor.copy_(
        #     gpu_tensor.reshape(cpu_tensor.shape),
        #     non_blocking=True
        # )
        # time_taken = (time.time() - start_time) * 1000
        # write_to_csv("offload", self.target_name, time_taken, tensor_size_mb)
        
        # gpu_tensor = gpu_tensor.offload(cpu_tensor)
        tmp_tensor = cpu_tensor.to("cuda", non_blocking=True)
        
        # if self.accumulating_offload_bytes:
        #     self.accumulated_offload_bytes += tensor_size_bytes
        #     print(f"Accumulated offload size {self.accumulated_offload_bytes / (1024 ** 2):.2f} MB")
        # print(f"Offloaded {self.target_name} to CPU, size: {tensor_size_bytes / (1024 ** 2):.2f} MB")
    ...
    def offload_grad(self):
        ...
        #     non_blocking=True,
        # )
        assert False, "not implemented yet" # Dat
        cpu_grad = self.storage.get_val(f"cpu_{self.target_name}").grad.data
        gpu_grad = self.storage.get_val(self.target_name).grad.data
        tensor_size_bytes = gpu_grad.numel() * gpu_grad.element_size()
        tensor_size_mb = tensor_size_bytes / (1024 ** 2)
        start_time = time.time()
        cpu_grad.copy_(
            gpu_grad.view(cpu_grad.shape),
            non_blocking=True
        )
        time_taken = (time.time() - start_time) * 1000  # in milliseconds
        # write_to_csv("offload", self.target_name, time_taken, tensor_size_mb)
        pass
    ...
    def __call__(self):
        # print(f"Offloading {self.target_name} to CPU on stream {self.stream}")
        with torch.cuda.stream(self.storage.gd[self.stream]):
            # pass
...
class Fct_prefetch(RK_Fct):
    ...
    def prefetch_tensor(self):
        ...
        cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}")
        if cpu_tensor.numel() == 0:
            print(f"Skipping prefetching {self.target_name} as CPU tensor is empty")
            return
        gpu_tensor = self.storage.get_val(self.target_name)
        
        # print(f"Prefetching {self.target_name} to GPU, gpu_shape: {gpu_tensor.shape}, cpu_shape: {cpu_tensor.shape}")
        
        # self.storage.add_val(self.target_name, cpu_tensor.to("cuda", non_blocking=True))
        # self.storage.add_val(self.target_name, cpu_tensor.to("cuda", non_blocking=False))
        # time.sleep(1)  # simulate some delay for prefetching
        
        # gpu_tensor.fetch(cpu_tensor)
        

        # cpu_tensor = self.storage.get_val(f"cpu_{self.target_name}").data
        # gpu_tensor = self.storage.get_val(self.target_name)
        
        # print(f"Fetching {self.target_name} to GPU, size: {cpu_tensor.numel() * cpu_tensor.element_size() / (1024 ** 2):.2f} MB")

        # # Tensor size in MB
        # tensor_size_bytes = cpu_tensor.numel() * cpu_tensor.element_size()
        # tensor_size_mb = tensor_size_bytes / (1024 ** 2)

        # start_time = time.time()
        # gpu_tensor.data.copy_(
        #     cpu_tensor.view(gpu_tensor.shape),
        #     non_blocking=True
        # )
        # time_taken = (time.time() - start_time) * 1000  # ms
        # # write_to_csv("prefetch", self.target_name, time_taken, tensor_size_mb)
        # if self.with_proxy:
        #     proxy_tensor = self.storage.get_val(f"_{self.target_name}")
        #     start_time = time.time()
        #     proxy_tensor.data = gpu_tensor.data
        #     time_taken = (time.time() - start_time) * 1000
        #     # write_to_csv("prefetch", f"{self.target_name}_proxy", time_taken, tensor_size_mb)
...
class Fct_add_tensor(RK_Fct):
    ...
    def __call__(self):
        # Dat this function is never used and should never be used for 
        print(f"Adding tensor {self.target_name} to storage with shape name {self.shape_name}")
        shape = self.storage.get_shape(self.shape_name)
...
class Fct_manager_alloc(RK_Fct):
    ...
    def __call__(self):
        ...
        for alloc, size in self.storage.manager.allocations:
            print(f"Creating allocation {alloc} with size {size} bytes ({size / (1024 ** 2):.2f} MB)")
            self.storage.add_val(
...
class Compiler:
    ...
    def compile_sched(self, op_sched: OpSchedule):
        ...
        for i, op in enumerate(op_sched.op_list):
            if op.disabled:
                print(f"Skipping disabled operation {op.name} of type {op.__class__.__name__}")
                continue
    ...
    def _activation_placehold(self, prep_op: Op, cluster, output_nodes):
        for anode in cluster.list_anodes:
            ...
            # Alloc empty activation tensors for offloaded activations.
            ...
            #     print(f"self.storage.dtype: {self.storage.dtype}, activation dtype: {dtype}")
