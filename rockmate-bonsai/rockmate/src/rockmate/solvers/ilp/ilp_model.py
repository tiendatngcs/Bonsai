...
class ModelPULP:
    ...
    def config_ilp(self):
        ...
        # for hcn in self.hgraph.list_HCNs:
        #     print(f"HCN {hcn.name}: is_fwd={hcn.is_fwd}, sub_cluster={hcn.sub_cluster}")
        #     if hcn.sub_cluster:
        #         sub_cns = hcn.sub_cluster.list_cnodes
        #         for sub_cn in sub_cns:
        #             print(f"  sub_HCN: {sub_cn.name}")
    ...
    def solve(self, solver=""):
        ...
        try:
            ...
            # )
            # solver = PULP_CBC_CMD(gapRel = 0.02)
            solver = GUROBI_CMD(msg=1, timeLimit=3600, threads=16)
        except:
