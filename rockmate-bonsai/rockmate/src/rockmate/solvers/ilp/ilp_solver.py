...
from .ilp_schedule import schedule, schedule_alt_2, schedule_alt_3, schedule_alt_4, schedule_alt_5, node_mapping, schedule_offload_all
...
from .ilp_segment_model import CheckpointSolverHandler, CheckpointGreedyHandler, CheckpointOpListHandler
import psutil
...
class HILP(Solver):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            # does not look like this solver is used anywhere
            solver = pulp.GUROBI_CMD(msg=1, warmStart=True)
            print(f"Using GUROBI to solve ILP")
            # solver = pulp.PULP_CBC_CMD(gapRel = 0.02)
    ...
    def solve_hg(
        ...
        def solve_md(
            ...
            if os.getenv("CUSTOM_ILP_SOLVER") == "1":
                print(f"Peak budget: {peak_budget}")
                trace_file_path = os.getenv("CUSTOM_TRACE_FILE_PATH", None)
                # static_allocation_MB = float(os.getenv("STATIC_ALLOCATION_MB", "0.0"))
                weight_MB = float(os.getenv("WEIGHT_MB", "0.0"))
                optimizer = os.getenv("OPTIMIZER", "adam")
                if trace_file_path is None:
                    raise ValueError("CUSTOM_TRACE_FILE_PATH environment variable not set")
                
                # check if trace_file_path exists
                if not os.path.exists(trace_file_path):
                    raise ValueError(f"Trace file path {trace_file_path} does not exist")

                segment_solver_handler = CheckpointSolverHandler(trace_file_path=trace_file_path, budget_in_bytes=peak_budget, weight_MB=weight_MB, optimizer=optimizer)
                node_mapping(md, cp_solver_handler= segment_solver_handler)
                segment_solver_handler.formulate_problem()
                segment_solver_handler.solve()
                # op_sched = schedule_alt_3(md, cp_solver_handler= segment_solver_handler)
                op_sched = schedule_alt_5(md, cp_solver_handler= segment_solver_handler)
                list_op_sched.append(op_sched)
                return list_op_sched, None
            
            if os.getenv("GREEDY_SEARCH") == "1":
                trace_file_path = os.getenv("CUSTOM_TRACE_FILE_PATH", None)
                module_op_trace_file_path = os.getenv("MODULE_OP_TRACE_FILE_PATH", None)
                if trace_file_path is None:
                    raise ValueError("CUSTOM_TRACE_FILE_PATH environment variable not set")
                if module_op_trace_file_path is None:
                    raise ValueError("MODULE_OP_TRACE_FILE_PATH environment variable not set")
                weight_MB = float(os.getenv("WEIGHT_MB", "0.0"))
                optimizer = os.getenv("OPTIMIZER", "adam")

                segment_search_handler = CheckpointGreedyHandler(trace_file_path=trace_file_path, module_op_trace_file_path=module_op_trace_file_path, budget_in_bytes=peak_budget, weight_MB=weight_MB, optimizer=optimizer)
                node_mapping(md, cp_solver_handler= segment_search_handler)
                segment_search_handler.search_for_checkpoint_segments()
                op_sched = schedule_alt_5(md, cp_solver_handler= segment_search_handler)
                list_op_sched.append(op_sched)
                return list_op_sched, None
            
            if os.getenv("RECOMPUTE_OP_LIST") == "1":
                trace_file_path = os.getenv("CUSTOM_TRACE_FILE_PATH", None)
                recompute_op_list_file_path = os.getenv("RECOMPUTE_OP_LIST_FILE_PATH", None)
                if trace_file_path is None:
                    raise ValueError("CUSTOM_TRACE_FILE_PATH environment variable not set")
                if recompute_op_list_file_path is None:
                    raise ValueError("RECOMPUTE_OP_LIST_FILE_PATH environment variable not set")
                weight_MB = float(os.getenv("WEIGHT_MB", "0.0"))
                optimizer = os.getenv("OPTIMIZER", "adam")
                
                op_list_handler = CheckpointOpListHandler(trace_file_path=trace_file_path, checkpoint_op_list_file_path=recompute_op_list_file_path, budget_in_bytes=peak_budget, weight_MB=weight_MB, optimizer=optimizer)
                node_mapping(md, cp_solver_handler= op_list_handler)
                op_sched = schedule_alt_5(md, cp_solver_handler= op_list_handler)
                list_op_sched.append(op_sched)
                return list_op_sched, None
                
            if os.getenv("NO_RECOMPUTE") == "1":
                op_sched = schedule_alt_2(md)
                list_op_sched.append(op_sched)
                return list_op_sched, None
            if os.getenv("OFFLOAD_ALL") == "1":
                op_sched = schedule_offload_all(md)
                list_op_sched.append(op_sched)
                return list_op_sched, None
            md.build()

            num_variables = len(md.md.variables())
            num_constraints = len(md.md.constraints)
