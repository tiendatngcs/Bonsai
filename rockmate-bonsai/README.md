...
```bash
export ROCKMATE_RECORD_FRAGMENTATION=1 # Print average memory usage and fragmentation of each operation at the end of training
export PRINT_ALLOCATED_MEMORY=1 # Print allocated memory after each operator execution
export CUSTOM_TRACE_FILE_PATH=<path to trace> # Path to trace file for segment_ilp
```
